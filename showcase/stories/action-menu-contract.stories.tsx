...
export const СОпаснымДействием: Story = {
  ...
  args: {
    groups: [
      ...
      {
        actions: [{ icon: <Trash />, label: "Удалить", onClick: noop }],
      },
    ],
  },
  parameters: {
    gap: {
      section: "4.3",
      text:
        "Компонент не умеет двух вещей из трёх. Признака destructive в типе Action " +
        "нет (action-menu.tsx:9-26) — опасность выражается только иконкой Trash и " +
        "меткой. Красного цвета нет: пункт красится теми же токенами, что и " +
        "остальные (action-menu.tsx:85, :102). Последним в списке действие стоит " +
        "потому, что так составлен этот вызов, а не потому, что компонент это " +
        "гарантирует. Подтверждение по контракту обязательно, но проверяется " +
        "человеком — здесь его нет, обработчик пустой.",
    },
  },
}
